...
const AssetLocations = () => {
  const { locations, addLocation, updateLocation, deleteLocation } = useFinance()
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingId) {
      updateLocation(editingId, { name, description })
  ...
  return (
    <div className="space-y-6 animate-slide-in">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
        <span className="text-primary-600 dark:text-primary-400 font-medium">Locais</span>
      ...
      <div className="bg-sky-50 dark:bg-sky-900/20 border border-sky-100 dark:border-sky-800 p-4 rounded-lg">
        <p className="text-sky-800 dark:text-sky-200 text-sm italic">
          Gerencie os locais onde os itens de patrimônio são inseridos.
        </p>
      </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
            <div className="bg-emerald-500 p-4">
              <h3 className="text-white font-bold flex items-center gap-2">
                ...
                {editingId ? 'Editar local' : 'Criar local'}
              </h3>
            </div>
