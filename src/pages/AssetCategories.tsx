...
const AssetCategories = () => {
  const { assetCategories, addAssetCategory, updateAssetCategory, deleteAssetCategory } = useFinance()
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingId) {
      updateAssetCategory(editingId, { name, description })
      showToast('Categoria atualizada com sucesso!', 'success')
      setEditingId(null)
    } else {
      addAssetCategory({ name, description })
  ...
  return (
    <div className="space-y-6 animate-slide-in">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
        <span className="text-primary-600 dark:text-primary-400 font-medium">Categorias</span>
      ...
      <div className="bg-sky-50 dark:bg-sky-900/20 border border-sky-100 dark:border-sky-800 p-4 rounded-lg">
        <p className="text-sky-800 dark:text-sky-200 text-sm italic">
          Gerencie as categorias de patrimônio.
        </p>
      </div>

      <div className="flex flex-col lg:flex-row gap-6">
        {/* Table Section */}
        <div className="flex-1 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden text-center lg:text-left">
          <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 p-4">
            <h2 className="text-lg font-bold text-sky-800 dark:text-sky-400">
              Resultados: {assetCategories.length}
            </h2>
          </div>
